...
class lcWebResponse extends lcResponse implements iKeyValueProvider, iDebuggable
{
	...
	*/
	...
	public function javascript($src, $type='text/javascript', $language='javascript')
	{
		return $this->setJavascript($src, $type, $language);
	}
	...
	*/
	public function setJavascript($src, $type='text/javascript', $language='javascript')
	{
		if (is_array($src)) {
			foreach($src as $s) {
				$this->setJavascript($s, $type, $language);
				unset($s);
			}
		} else {
			$this->javascripts[$src] = array('src' => $src, 'type' => $type, 'language' => $language);
	...
	*/
	public function prependJavascript($src, $type='text/javascript', $language='javascript')
	{
		$new = array();
		$new[$src] = array('src' => $src, 'type' => $type, 'language' => $language);
	...
	*/
	...
	private function processViewConfiguration() {
		...
		if ($javascripts)
		{
			$config_js = (array)$this->configuration['view.javascripts'];

			foreach ($config_js as $js)
			{
				// relative or absolute path
	...
	*/
	private function processHtmlContent($content)
	{
		...
		if ($javascripts)
		{
			...
			{
				$head[] =
				'<script type="' . $data['type'] . '" src="' . $data['src'] . '"></script>';
